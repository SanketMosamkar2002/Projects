...
namespace LibraryManagementSystem
{
    public partial class publishermanagement : System.Web.UI.Page
    {
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Write("<script>alert('Publisher already exists with the same id');</script>");
            }
            else
            {
                addNewPublisher();
        ...
        void deletePublisherByID()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("DELETE publisher_master_table WHERE publisher_id='" + TextBox1.Text.Trim() + "'", con);
                cmd.ExecuteNonQuery();
                con.Close();
